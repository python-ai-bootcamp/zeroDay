...
ExecStart= /bin/sh -c 'DOMAIN_NAME="zerodaybootcamp.xyz" /usr/local/bin/uvicorn main:app --host 0.0.0.0 --port 8000 2>&1 >> /opt/logs/zeroDay_\$\$(date +%%Y_%%m_%%d__%%H_%%M_%%S).log'
Restart=on-failure
